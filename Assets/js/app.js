			...
			str += `
					<tr>
						<th class="cart-prod" scope="row">
							...
							<div class="cart-text">
								<h5>${myCart[i].name}</h5>
								<h6>A-hinta : ${myCart[i].price}</h6>
								<span id="${myCart[i].id}" onclick="removeFromCart(this.id)">Poista</span>
							</div>
						</th>
						<td>${myCart[i].quantity}</td>
						<td>${parseFloat((myCart[i].price).substring(0, myCart[i].price.length-2)) * (myCart[i].quantity)} €</td>
					</tr>
    `;
		}
...
function showTotalPrice() {
	...
	if (myCart == null) {
		...
		let sum = 0;
		for (let i = 0; i < myCart.length; i++) {
			sum += parseFloat((myCart[i].price).substring(0, myCart[i].price.length-2)) * (myCart[i].quantity);
